...
| `eyelashes`  | `true`, `false` — round-family eyes only, and only with more than one eye: each eye gets one lash that leaves its upper-outer edge along the outline, then flicks up |
