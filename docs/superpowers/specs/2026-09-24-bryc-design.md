...
| `eyelashes`  | `true`, `false` — round-family eyes only, and only with more than one eye: each eye gets one bold, tapered lash that leaves its upper-outer edge along the outline, then flicks up; near the head's sides lashes may sweep past its outline |
