...
| `tall`       | `true`, `false` — head extends 80 units upward and is 30 units narrower; the face, ears and rivets stay anchored to the head's bottom, so it reads as a taller forehead |
| `eyes`       | `round`, `oval` (round squashed to 2/3 height), `focused` (oval with inner ends tilted down 20°; a single eye stays level), `visor` |
| `eyecount`   | `1`, `2`, `4`, `6` — round-family eyes (round, oval, focused): one centered eye, or 1–3 stacked pairs, smaller as there are more. Resolved either way; the web page greys it out unless the eyes are round, oval or focused |
| `eyesize`    | `1`–`4` — round-family eyes only: radius 25 / 40 / 58 / 95, capped at the eye count's size (6 eyes → 1, 4 → 2, 2 → 3, 1 → 4); eyes keep their positions |
| `eyestyle`   | `glower` (HAL-style: metal ring, black lens, glow lit in the center), `bright` (glow-filled, glowing over its outline, with a hot spot), `dead` (black lens with a glint) — round-family eyes only |
| `eyelashes`  | `true`, `false` — round-family eyes only, and only with more than one eye: each eye gets one bold, tapered lash that leaves its upper-outer edge along the outline, then flicks up. With lashes, eyes move inward as far as needed to keep every lash inside the head (never closer than 20 units apart; only then may a lash overhang) |
